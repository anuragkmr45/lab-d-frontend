...
const MyBookingCard = ({ bookingData }: { bookingData: BookingDataType }) => {

    const handleDeleteBooking = async () => {
        try {
            const res = await deleteBooking(bookingData.bookingId);
    ...
    return (
        ...
        //     </div>
        // </div>
        <div className="w-full border-2 shadow-md rounded-xl border-[#d7d6d6] p-4 m-2">
            <h1>My Booking Card</h1>
            <p>bookingId: {bookingData.bookingId}</p>
            <p>userId: {bookingData.userId}</p>
            <p>date: {bookingData.date}</p>
            <p>time: {bookingData.time}</p>
            <p>reportStatus: {bookingData.reportStatus}</p>
            <p>paymentStatus: {bookingData.paymentStatus}</p>
            <p>collectionLocation: {bookingData.collectionLocation}</p>
            {/* <p>bookingId: </p> */}
